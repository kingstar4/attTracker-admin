...
import { Mail, Phone, ShieldCheck, UserCheck, UserRound } from "lucide-react";
import {
  OwnerProfileUpdatePayload,
  useOwnerProfileStore,
} from "@/store/useOwnerProfileStore";
...
export function OwnerProfile() {
  ...
  const [formValues, setFormValues] = useState<OwnerProfileUpdatePayload>({
    phone_number: "",
  ...
  const fullName = useMemo(() => {
    const fallback = account?.email ?? "Owner account";
  ...
  }, [profile?.first_name, profile?.last_name, account?.email, profile]);

  useEffect(() => {
    console.log("OwnerProfile component mounted, calling fetchProfile");
    void fetchProfile();
  }, []);
  ...
  const handleChange =
    (field: keyof OwnerProfileUpdatePayload) =>
    ...
    <div className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="flex items-start gap-3 rounded-lg border p-3">
        ...
        <div>
          ...
          <p className="text-sm font-medium">{account?.role ?? "Owner"}</p>
        </div>
      </div>
      <div className="flex items-start gap-3 rounded-lg border p-3">
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="space-y-1">
        ...
        <p className="text-sm text-muted-foreground">
          Review and update the information shared with your supervisors and
          employees.
        </p>
      </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            This information helps supervisors reach you during incidents.
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Keep these details handy when working with supervisors.
          </CardDescription>
        </CardHeader>
