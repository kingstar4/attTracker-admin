...
import { Users, Clock, UserCheck, TrendingUp, Calendar, MapPin } from "lucide-react"
...
export function AdminDashboard() {
  ...
  const employeesOnBreak = employees.filter((emp) => emp.status === "break").length
  ...
  const enrollmentRate = Math.round(
    (employees.filter((emp) => emp.fingerprintEnrolled).length / employees.length) * 100,
  )
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Fingerprint Enrollment</span>
                <span className="font-medium">{enrollmentRate}%</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Estimated Hours Today</span>
